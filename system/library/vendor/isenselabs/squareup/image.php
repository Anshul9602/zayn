...
class Image extends Library {
    ...
    protected function initOldImage() {
        ...
        if (is_null($this->old)) {
            throw new Exception\Image('Cannot load image: ' . $this->source_image_path);
        }
    }
    ...
    protected function initInfo() {
        ...
        if (is_null($this->mime)) {
            throw new Exception\Image('Cannot determine the image mime: ' . $this->source_image_path);
        }

        if (empty($this->width) || empty($this->height)) {
            throw new Exception\Image('Cannot determine image dimensions (WxH): ' . $this->source_image_path);
        }
    }

    protected function init() {
        if (!extension_loaded('gd')) {
            throw new Exception\Image('PHP GD is not installed!');
        }

        if (!is_readable($this->source_image_path) || !is_file($this->source_image_path)) {
            throw new Exception\Image('Cannot read the source image: ' . $this->source_image_path);
        }
    }
        ...
        foreach ($parts as $part) {
            ...
            if (!is_dir($new_dir)) {
                if (false === mkdir($new_dir, 0755)) {
                    throw new Exception\Image('Cannot create dir: ' . $new_dir);
                }
            }
        }

        if (!is_writable($new_dir)) {
            throw new Exception\Image('Cannot write to dir: ' . $new_dir);
        }
