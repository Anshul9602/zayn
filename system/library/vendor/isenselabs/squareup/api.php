...
class Api extends Library {
    ...
    public function updateWebhookPermissions($location_id, $permissions) {
        ...
        if (!empty($return['type']) && !empty($return['message']) && $return['type'] == 'bad_request') {
            throw new Exception\Api($this->registry, $return['message']);
        }
    }
    ...
    public function uploadImage($filepath, $item_id) {
        ...
        $request_data = array(
            'method' => 'POST',
            'endpoint' => sprintf(self::ENDPOINT_UPLOAD_IMAGE, $this->config->get('squareup_location_id'), $item_id),
            'no_version' => true, // The version is included in the endpoint
        ...
        if (empty($return['url']) || empty($return['id'])) {
            throw new Exception\Api($this->registry, "Cannot upload image for " . $item_id);
        }
    ...
    public function authLink($client_id) {
        ...
        $redirect_uri = str_replace('&amp;', '&', $this->url->link($this->config->get('squareup_imodule_route_payment') . '/oauth_callback', 'token=' . $this->session->data['token'], $this->getUrlSsl()));

        $this->session->data['squareup_oauth_redirect'] = $redirect_uri;
    ...
    public function exchangeCodeForAccessToken($code) {
        $request_data = array(
            ...
            'no_version' => true,
            'parameters' => array(
                'client_id' => $this->session->data['squareup_connect']['squareup_client_id'],
                'client_secret' => $this->session->data['squareup_connect']['squareup_client_secret'],
    ...
    public function refreshToken() {
        $request_data = array(
            'method' => 'POST',
            'endpoint' => sprintf(self::ENDPOINT_REFRESH_TOKEN, $this->config->get('squareup_client_id')),
            'no_version' => true,
            'auth_type' => 'Client',
            'token' => $this->config->get('squareup_client_secret'),
            'parameters' => array(
    ...
    public function createOrder($order_id, $rounding_adjustment = 0, $resync_on_price_difference = false) {
        ...
        foreach ($order_products as $product) {
            ...
            if (false === $variation = $this->getProductVariation($product['product_id'], $product['order_product_id'])) {
                ...
                foreach ($this->imodule_model_payment->getOrderOptions($order_id, $product['order_product_id']) as $option) {
                    if (empty($option['product_option_value_id'])) {
                        continue;
                    }

                    if ($this->config->get('squareup_ad_hoc_sync')) {
        ...
        $order_totals = $this->imodule_model_payment->getOrderTotals($order_id);
        ...
        if ($price_diff != 0 && $resync_on_price_difference) {
            throw new Exception\Api($this->registry, "SQUAREUP: Re-syncing the items due to price difference!");
        }
    ...
    public function getProductVariation($product_id, $order_product_id) {
        if ($this->config->get('squareup_sync_source') != 'opencart') {
            return false;
        }
    ...
    public function getLocationCurrency($default) {
        $locations = $this->config->get('squareup_locations');
        $location_id = $this->config->get('squareup_location_id');
    ...
    protected function api($request_data) {
        ...
        if ($result) {
            ...
            if (!empty($info['http_code'])) {
                throw new Exception\Api($this->registry, "CURL error. Info: " . print_r($info, true), true);
            } else {
                throw new Exception\Network("Temporary network error. Please try again later.");
            }
        }
    }
    ...
    protected function authState() {
        if (!isset($this->session->data['squareup_oauth_state'])) {
            $this->session->data['squareup_oauth_state'] = bin2hex(openssl_random_pseudo_bytes(32));
        }

        return $this->session->data['squareup_oauth_state'];
    }
}