...
class Diff extends Library {
    ...
    public function getLocationOverrideOrDefault($variation_data, $key) {
        if (isset($variation_data['location_overrides']) && is_array($variation_data['location_overrides'])) {
            foreach ($variation_data['location_overrides'] as $location_override) {
                if ($location_override['location_id'] == $this->config->get('squareup_location_id')) {
    ...
    public function appendCurrentLocation($locations) {
        if (!in_array($this->config->get('squareup_location_id'), $locations)) {
            $locations[] = $this->config->get('squareup_location_id');
        }
    ...
    public function determineDeleteOrDisassociate(&$row, &$delete_ids, &$disassociate_ids, &$last_id_map) {
        ...
        foreach ($present_at_location_ids as $index => $location_id) {
            if ($location_id == $this->config->get('squareup_location_id')) {
        ...
        if (!empty($row['present_at_all_locations']) && !$has_been_unset && $row['type'] != 'CATEGORY' && !in_array($this->config->get('squareup_location_id'), $absent_at_location_ids)) {
    ...
    protected function initDiffId() {
        ...
        $this->{$this->config->get('squareup_imodule_model_payment')}->editSquareSetting(array(
            'squareup_last_sync_diff_id' => $this->diffId
        ));
    }
    ...
    protected function pushDelayedInsertObject(&$object) {
        ...
        if (in_array($this->config->get('squareup_location_id'), $absent_at_location_ids)) {
            return false;
        }

        if (!empty($present_at_location_ids) && !$present_at_all_locations && !in_array($this->config->get('squareup_location_id'), $present_at_location_ids)) {
            return false;
        }
            ...
            if ($num_rows > 0) {
                foreach ($result->rows as $row) {
                    ...
                    if ($row['status'] == self::STATUS_ERROR) {
                        ...
                        throw new Exception\Api($this->registry, $row['message']); // Terminate the whole script and return the error.
                    }
                }
    ...
    protected function checkTimeOut() {
        ...
        if ($this->{$this->config->get('squareup_imodule_model_payment')}->cronHasTimedOut()) {
    ...
    protected function uploadImages($product_id) {
        ...
        if ($result->num_rows) {
            ...
            if (empty($data['image_url']) || $data['image_url'] != $result->row['url']) {
                ...
                $resizer = new Image($this->registry, $image_path, $result->row['square_id']);
