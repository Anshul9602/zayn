...
class ModelCheckoutOrder extends Model
{
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false)
	{
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$order_product_query = $this->db->query("SELECT op.*, p.image FROM " . DB_PREFIX . "order_product op LEFT JOIN oc_product p ON op.product_id = p.product_id WHERE op.order_id = '" . (int)$order_id . "'");
				...
				foreach ($order_product_query->rows as $product) {
					...
					$data['products'][] = array(
						...
						'image'   =>  'https://www.zaynjewels.com/image/'.$product['image'],
	...
	public function test(){
		...
		print_r($data['diamondProducts']);
		
		if (in_array(1063, $data['diamondProducts'])) {
			print_r('ssss');
		}
