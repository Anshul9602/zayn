...
use vendor\isenselabs\Squareup\Compatibility\Model\Payment as Model;
...
class ModelExtensionPaymentSquareup extends Model {
    ...
    public function newTaxRatesEmail($tax_rates) {
        ...
        $url = $this->url->link($this->imodule_route_payment . '/info', 'squareup_settings=1&cron_token=' . $this->config->get('squareup_cron_token'), $this->getUrlSsl());
    ...
    public function oneCronStandardPeriodHasPassed() {
        ...
        if ($time > 0 && (time() - $time > 60 * (int)$this->config->get('squareup_cron_standard_period'))) {
            return true;
        }
    ...
    public function setEndCronFlags($flag) {
        if ($flag != self::CRON_ENDED_FLAG_INVALID) {
            $this->editSquareSetting(array(
                'squareup_cron_is_running' => '0',
                'squareup_cron_is_on_demand' => '0',
                'squareup_cron_ended_at' => time(),
                'squareup_cron_ended_type' => $flag
            ));
        }
    }

    public function cronHasTimedOut() {
        $time = (int)$this->getSquareSetting('squareup_cron_started_at');

        switch ($this->getSquareSetting('squareup_cron_started_type')) {
            case self::CRON_STARTED_FLAG_STANDARD :
            case self::CRON_STARTED_FLAG_ON_DEMAND : {
                // Standard period minus 5 minutes
                $timeout = (int)$this->getSquareSetting('squareup_cron_standard_period') * 60 - (int)$this->config->get('squareup_cron_timeout_buffer');
    ...
    public function recurringPayments() {
        return (bool)$this->config->get('squareup_recurring_status');
    }
    ...
    public function createRecurring($recurring, $order_id, $description, $reference) {
        if (version_compare(VERSION, '2.0.0.0', '<')) {
            ...
            $trial_duration = (bool)$recurring['recurring_trial'] ? (int)$recurring['recurring_trial_duration'] : 0;

            $this->db->query("INSERT INTO `" . DB_PREFIX . "order_recurring` SET `order_id` = '" . (int)$order_id . "', `created` = NOW(), `status` = '" . self::RECURRING_ACTIVE . "', `product_id` = '" . (int)$recurring['product_id'] . "', `product_name` = '" . $this->db->escape($recurring['name']) . "', `product_quantity` = '" . $this->db->escape($recurring['quantity']) . "', `profile_id` = '" . (int)$recurring['profile_id'] . "', `profile_name` = '" . $this->db->escape($recurring['profile_name']) . "', `profile_description` = '" . $this->db->escape($description) . "', `recurring_frequency` = '" . $this->db->escape($recurring['recurring_frequency']) . "', `recurring_cycle` = '" . (int)$recurring['recurring_cycle'] . "', `recurring_duration` = '" . (int)$recurring['recurring_duration'] . "', `recurring_price` = '" . (float)$recurring['recurring_price'] . "', `trial` = '" . (int)$recurring['recurring_trial'] . "', `trial_frequency` = '" . $this->db->escape($recurring['recurring_trial_frequency']) . "', `trial_cycle` = '" . (int)$recurring['recurring_trial_cycle'] . "', `trial_duration` = '" . (int)$trial_duration . "', `trial_price` = '" . (float)$recurring['recurring_trial_price'] . "', `profile_reference` = '" . $this->db->escape($reference) . "'");
    ...
    public function validateCRON() {
        ...
        if (isset($this->request->get['cron_token']) && $this->request->get['cron_token'] == $this->config->get('squareup_cron_token')) {
            return true;
        }
    ...
    public function updateToken() {
        try {
            ...
            if (!isset($response['access_token']) || !isset($response['token_type']) || !isset($response['expires_at']) || !isset($response['merchant_id']) || $response['merchant_id'] != $this->config->get('squareup_merchant_id')) {
    ...
    public function nextRecurringPayments() {
        ...
        if (version_compare(VERSION, '2.0.0.0', '<')) {
            $recurring_sql = "SELECT * FROM `" . DB_PREFIX . "order_recurring` `or` INNER JOIN `" . DB_PREFIX . "squareup_transaction` st ON (st.transaction_id = `or`.profile_reference) WHERE `or`.status='" . self::RECURRING_ACTIVE . "'";
        } else {
        ...
        foreach ($this->db->query($recurring_sql)->rows as $recurring) {
            ...
            if ($recurring['merchant_id'] != $this->config->get('squareup_merchant_id')) {
                continue;
            }
    ...
    public function getExpiringAuthorizedTransactions() {
        ...
        return $this->db->query("SELECT * FROM `" . DB_PREFIX . "squareup_transaction` WHERE transaction_type='AUTHORIZED' AND created_at < '" . $two_days_ago . "' AND merchant_id='" . $this->db->escape($this->config->get('squareup_merchant_id')) . "'")->rows;
    }
    ...
    private function getLastSuccessfulRecurringPaymentDate($order_recurring_id) {
        if (version_compare(VERSION, '2.0.0.0', '<')) {
            return $this->db->query("SELECT created FROM `" . DB_PREFIX . "order_recurring_transaction` WHERE order_recurring_id='" . (int)$order_recurring_id . "' AND type='" . self::TRANSACTION_PAYMENT . "' ORDER BY created DESC LIMIT 0,1")->row['created'];
        } else {
    ...
    private function paymentIsDue($order_recurring_id) {
        ...
        if (!$this->getTotalSuccessfulPayments($order_recurring_id)) {
            if (version_compare(VERSION, '2.0.0.0', '<')) {
                $previous_time = strtotime($recurring_info['created']);
            } else {
    ...
    public function getSquareSetting($key) {
        $group_or_code = version_compare(VERSION, '2.0.0.0', '<=') ? 'group' : 'code';

        $result = $this->db->query("SELECT `value`, `serialized` FROM `" . DB_PREFIX . "setting` WHERE `" . $group_or_code . "`='squareup' AND `key`='" . $this->db->escape($key) . "'");
    ...
    public function editSquareSetting($settings) {
        ...
        foreach ($settings as $key => $value) {
            $this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `" . $group_or_code . "`='squareup' AND `key`='" . $this->db->escape($key) . "'");

            $this->db->query("INSERT INTO `" . DB_PREFIX . "setting` SET `" . $group_or_code . "`='squareup', `key`='" . $this->db->escape($key) . "', `value`='" . $this->db->escape($value) . "', serialized=0, store_id=0");
        }
    }
