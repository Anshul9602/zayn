...
class ModelExtensionPaymentSquareup extends Model {
    ...
    public function getOrderStatusId($order_id, $transaction_status = null) {
        if ($transaction_status) {
            return $this->config->get('squareup_status_' . strtolower($transaction_status));
        } else {
    ...
    public function getOrderStoreId($order_id) {
        $sql = "SELECT store_id FROM `" . DB_PREFIX . "order` WHERE `order_id`=" . (int)$order_id;
    ...
    public function getAdminURLTransaction() {
        return $this->url->link($this->imodule_route_payment . '/transaction_info', '&squareup_transaction_id=%s%s', $this->getUrlSsl());
    }

    public function getAdminURLSettings() {
        return $this->url->link($this->imodule_route_payment, '%s', $this->getUrlSsl());
    }
    ...
    public function getTransactionStatus($transaction) {
        ...
        $result['is_merchant_transaction'] = $transaction['merchant_id'] == $this->config->get('squareup_merchant_id') && $this->inLocations($transaction['location_id']);
        ...
        if ($result['is_merchant_transaction'] && !in_array($transaction['transaction_type'], array('VOIDED', 'FAILED')) && !$this->isTransactionFullyRefunded($transaction)) {
            ...
            if ($updated_status == 'VOIDED') {
                ...
                $result['order_history_data'] = array(
                    ...
                    'store_id' => $this->getOrderStoreId($transaction['order_id']),
    ...
    public function createEvents() {
        $events = $this->config->get('squareup_imodule_events');

        $this->load->model($this->imodule_event_route);

        foreach ($events as $trigger => $action) {
            $this->{$this->imodule_event_model}->addEvent('squareup', $trigger, $action);
        }
    }
    ...
    public function dropEvents() {
        ...
        $this->{$this->imodule_event_model}->deleteEvent('squareup');
    }
    ...
    public function setupApplePayDomainVerificationFile() {
        ...
        $copy =
            ...
            @copy(
                DIR_SYSTEM . 'library/vendor/isenselabs/squareup/apple_pay/apple-developer-merchantid-domain-association',
                $target_dir . '/apple-developer-merchantid-domain-association');

        if (!$copy) {
            throw new vendor\isenselabs\Squareup\Exception\Api($this->registry, sprintf($this->language->get("error_cannot_copy_verification"), $target_dir));
        }
    ...
    private function geoZonePerCountryZone($iso_code_3, $country_name) {
        ...
        foreach ($this->model_localisation_zone->getZonesByCountryId($country_id) as $zone) {
            ...
            if (!$this->existsGeoZoneByName($name)) {
                $data = array(
                    'name' => $name,
                    'description' => '(Square) Created on: ' . date($this->language->get('date_format_short') . ' ' . $this->language->get('time_format')),
