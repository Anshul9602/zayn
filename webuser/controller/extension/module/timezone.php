...
class ControllerExtensionModuleTimezone extends Controller {
	...
	public function index() {
	  if (version_compare($this->version, $this->config->get('timezone_version'), 'gt')) $this->install();
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/module/timezone', 'token=' . $this->session->data['token'] . '&refreshModCache=1', 'SSL'));
		}
    ...
    if (isset($this->request->get['redirect'])) $this->response->redirect($this->url->link('extension/module/timezone', 'token=' . $this->session->data['token'] . '&refreshModCache=1', 'SSL'));
	} //install end
} //class end